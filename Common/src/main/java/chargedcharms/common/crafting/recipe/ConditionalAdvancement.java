...
public class ConditionalAdvancement
{
    ...
    public static JsonObject processConditional(JsonObject json, ICondition.IContext context) {
        ...
        if (entries == null)
        {
            return ChargedCharmsCrafting.processConditions(json, "conditions", context) ? json : null;
        }
        ...
        for (JsonElement ele : entries)
        {
            ...
            if (ChargedCharmsCrafting.processConditions(GsonHelper.getAsJsonArray(ele.getAsJsonObject(), "conditions"), context))
        ...
        public JsonObject write()
        {
            ...
            for (int x = 0; x < conditions.size(); x++)
            {
                ...
                for (ICondition c : conditions.get(x))
                    conds.add(ChargedCharmsCrafting.serialize(c));
